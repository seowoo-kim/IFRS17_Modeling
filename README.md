...
개인적으로 테스트하며 탐구한 내용이므로 특정 보험사의 메타와 데이터모델링과는 상이함.  
...
1. 보험사업의 수익 인식이 기존의 보험료나 투자수입금 유입이 아니라, 계약과 시장금융변수와 통계 등 다양한 기초 정보에 근거하여 계산 해야함. <br/>   시중의 정보와 기업고유자료를 잘 쌓고 정제해서 합리적인 추정으로 미래의 보험서비스 제공에 따른 수익인식을 해야하여 데이터가 중요해짐.<br/>
2. 회계단계가 세분화되고 통계적 추정이 많아지면서 변동성 설명을 위한 세분화된 재무정보와 가정의 분석단위 세분화 체계가 필요해짐.<br/>
3. 미래의 현금흐름을 최선추정하기 위해서 다양한 가정과 복잡한 예측모델과 식을 사용하게 됨.<br/>   (경제가정, 지급률, 계약자행동, 시나리오 확률가중평균, 보험서비스와 관련된 모든 부문 현금흐름 추정 등)<br/>
4. 통계와 가정의 전제하에 회계처리가 이루어지므로 회계정책에 대한 통제와 변경사항을 엄격하게 관리해야 함. <br/>
5. 대량의 데이터와 가정(계약자정보, 경제가정, 계리가정, 사업비가정, 운영정책가정 등)이 얽혀 여러단계로 계산되기에 사소한 오류로 잘못된 회계계산이 이루어 질 수 있음.<br/>
<br/>
