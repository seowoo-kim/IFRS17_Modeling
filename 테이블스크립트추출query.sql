...
--일부 테이블 임시테이블로 생성 시, 개발계 followup table생성 시 이용
...
WHERE TABLE_NAME IN ('SPEL_RKRT_INF')     -----------------** A.TABLE_NAME 수정할것
UNION ALL
...
SELECT LEVEL AS ADD_M FROM DUAL CONNECT BY LEVEL < 61)) A, ALL_TABLES B    -----------------** 파티션길이 50부분 수정할것, 필요에따라서 파티셔닝 단위를 맞출것, 세그먼트낭비 피하기
WHERE B.TABLE_NAME IN ('SPEL_RKRT_INF')     -----------------** A.TABLE_NAME 수정할것
)
...
WHERE A.TABLE_NAME ='TT_RSUR_TRT_BAS_1212' AND A.OWNER = B.INDEX_OWNER AND A.INDEX_NAME = B.INDEX_NAME AND A.TABLE_NAME = B.TABLE_NAME    -----------------** A.TABLE_NAME 수정할것
UNION ALL
...
FROM ALL_INDEXES A ,
     ...
     GROUP BY INDEX_OWNER, INDEX_NAME, TABLE_NAME) B 
WHERE ((A.INDEX_NAME LIKE '%_PK') OR (A.TABLE_NAME LIKE 'PK_%')) AND A.TABLE_NAME ='TT_RSUR_TRT_BAS_1212' AND A.OWNER = B.INDEX_OWNER AND A.INDEX_NAME = B.INDEX_NAME AND A.TABLE_NAME = B.TABLE_NAME) A    -----------------** A.TABLE_NAME 수정할것
...
WHERE TABLE_NAME ='TT_RSUR_TRT_BAS_1212' AND TABLE_TYPE ='TABLE'    -----------------** A.TABLE_NAME 수정할것
UNION ALL
...
ALL_TAB_COLUMNS B WHERE A.TABLE_NAME ='TT_RSUR_TRT_BAS_1212' AND A.TABLE_NAME = B.TABLE_NAME AND A.OWNER = B.OWNER AND A.COLUMN_NAME= B.COLUMN_NAME)   -----------------** A.TABLE_NAME 수정할것
ORDER BY OWNER, TABLE_NAME, NUM;


--4.GRANT
GRANT SELECT, DELETE, UPDATE, INSERT ON CF.RSUR_TRT_BAS TO FMS;
GRANT SELECT, DELETE, UPDATE, INSERT ON CF.RSUR_TRT_BAS TO R_SYS_CF_DML;
GRANT SELECT ON CF.RSUR_TRT_BAS TO R_SYS_CF_SIMU; 
GRANT SELECT, DELETE, UPDATE, INSERT ON CF.RSUR_TRT_BAS TO R_SYS_CF_SIMU_DML;
